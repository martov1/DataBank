...
## 4. The common-emitter amplifier 
...
* Conozco como obtener las impedancias de entrada y salida $Z_{in}$ y $Z_{out}$
	* Esto va a determinar $R_c$
	* Esto va a afectar $R_1$ y $R_2$
* Conozco como calcular el voltage gain $A_v$
	* Esto va a determinar $\frac{R_c}{R_e}$
* Se que debo mantener $V_e>1v$ para mantener constante la corriente $I_e$.
	* Esto va a determinar $R_e$
*  Mantengo$V_c=\frac{V_{cc}}{2}$ para permitir un buen swing de la señal
	* Esto va a determinar $R_c$
* La **quiescent curent** $I_c$ no necesita ser muy alta, por que amplifico voltaje, no potencia.
	* un  $I_c$ muy alto desperdicia corriente y genera calor
	* un  $I_c$ muy bajo implica un $R_c$ alto, y eso disminuye la impedancia de salida $Z_{out}$
	* $I_c$ esa limitada por la corriente maxima del transistor y la potencia maxima disipada de los resistores
...
Con todas esas relaciones, dependiendo de cual sea nuestro objetivo, podemos diseñar el amplificador.

**Es importante notar que la forma como diseñaremos el amplificador depende de:**


* **Que OBJETIVOS queremos lograr:**
	* Minima corriente $I_{out}$ que necesitamos de nuestro amplificador, depende de la impedancia $Z_{out}$
	*  Ganancia que queremos obtener $A_v$
* **Que LIMITACIONES tenemos**
	* Maxima corriente $I_c$ del transistor
	* Maxima corriente $I_c$ que puede otorgarnos nuestra fuente $V_{cc}$
	* Maxima corriente $I_{in}$que puede otorgarnos nuestra señal $V_{in}$, que depende de la impedancia $Z_{in}$
	* Cuanta potencia estoy dispuesto a desperdiciar en forma de calor o en cuanto a cantidad de corriente

![](https://i.imgur.com/zt06HQK.png)

.
* [ejemplo de diseño 1](http://www.phy.olemiss.edu/~cremaldi/PHYS321/CommonEmitterAmp%20Design-HW.pdf)
* [Ejemplo de diseño 2](https://learnabout-electronics.org/Amplifiers/amplifiers21.php)
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTM0MTY0MTU5NywtMTk5NDUwMDk0OSw3Mj
A3NzUwMjEsLTExNDc4MDA5MDIsMTg2NDc5NDQ1NywtMTcxNzUy
NTY5OSwxNTUzMTM4NDEwLC0xOTM4NTIyMzcyLDgxNTk2MTA3OC
wzNTYyNTgyMzQsLTE5ODYyMzU3NTQsOTIyMTAwMDQ4LC05NDY5
NDkwMzgsMTgyNjI4NTg2LC00OTUwOTkwOTAsMTM4ODM0ODY5OS
w0OTgxMDM4ODgsLTYzOTI4MjA4OSwtOTgyODg4ODgsLTYxODMx
NzMwN119
-->