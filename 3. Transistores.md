...
* **quede en impedancia de salida/entrada**
* 
**TODO**
...
Es posible que aun cuando usamos el transistor como un switch, la corriente $I_b$ siga siendo muy alta paa nuestro circuito de mando (un integradito por ejemplo)

Para rescatar esta situacion, podemos usar un **emitter follower** que nos **baje la impeancia de la carga** y nos permita **proveer mas corriente para accionar el transistor que funciona como switch** 
...
2. Conectar esa tension de referencia a un emitter-follower
...
### Emitter-follower  coupled amplifier - WIP

#### Intro
[ver que onda aca](https://www.youtube.com/watch?v=vmTE_Qf2K9M)



_**nota:** este sistema sa el mimo nament ue el mpca de enin_


podemos usar uestro emiter-follower para *reducir la impedanca de a señal AC

**IMPORTANTE:**
El transistor no puede amplificar la parte negativa de la onda, entonces
* Cuando salga del transistor le saquemos la parte positiva, para que sea AC de nuevo
* Este encima de los $0.7v$



Para eso:
1  Creo un voltaje de referencia para alimentar la base del transistor usando un **voltage divider** usando $R_1$ y $R_2$
2 Acoplo la señal AC a esa tension de referencia usando un **coupling capacitor**
3. Ahora que tengo mi señal montada sobre una señal DC positiva, ya no tengo problemas para amplificarla con el transistor. asi que la uso para **alimentar el emitter-foll**
	3. Del emisor sale una corriente correspondiente a $I_b*\beta$, donde $I_b$ sale casi totalmente del divo e tension y no de la señal original.
4. **Extraemos la parte AC** de la señal con un coupling capacitor y la usamos para mover una carga.

[ejemplo](https://tinyurl.com/yy65olvc)
![](https://i.imgur.com/CJuEM37.png)


#### Impedancia de entrada $Z_{in}$ - WIP

[REVISAR](https://electronics.stackexchange.com/questions/547108/trouble-understanding-output-impedance-of-common-colector?noredirect=1#comment1422720_547108)
[ver](https://www.youtube.com/watch?v=Xjf1bMW-NZo&list=PLvs4GyVYMn63eWvF5v8U5NvYVbzPXEf91&index=19)

v//wtsouueomwaktpjTS3xKw&list=PLZvLSclgk4yIxWlpkNfiJKG4eOxZUyvfi&index=18)
[ver2](https://www.youtube.com/watch?v=YWiEicdyTCQ&list=PLZvLSclgk4yJkRHB_eJ10zo2exTY_irR0&index=4)
[ver3](https://www.youtube.com/watch?v=tSJC-JFSRP0&list=PLZvLSclgk4yJkRHB_eJ10zo2exTY_irR0&index=9)
[ver4](https://www.youtube.com/watch?v=rMhT7zT3g2c&list=PLZvLSclgk4yJkRHB_eJ10zo2exTY_irR0&index=11)

[ESTO SE VE FACIL DE SEGUIR Y EXCELENTE](https://www.youtube.com/watch?v=sydqHop-e2k&list=PL3QYrR_osdRrcnBNwWBVEIXLWF1ccEwzs)



**REVISAR IMPEDANCIA DE ENTRADA... CREO QUE ESTA MAL!**


>**LA SOLUCION:** la impedancia de salida es simplemente la resistencia de thevenin que ve la carga. pero para hacerla tengo que modelar el transistor de otra manera
>**TIENE SENTIDO!:** al final del dia, mi ampli no es mas que **una fuente de thevenin y una resistencia conectados a mi carga!** osea que lo que va a limitar la corriente a mi carga ($Z_{out}$) va a ser la resistencia de thevenin ($R_{th}$)

Podemos identificar cuales van a ser las impedancias de entrada $Z_{in}$ y $Z_{out}$ facilmente.



* $Z_{in}$ va a ser la resistencia que ve la señal de entrada, en este caso vemos que la corriente **AC** de la señal puede ir 
...
![](https://i.imgur.com/HU0ti6w.png)

Entonces la señal de entrada puede irse por cualquiera de esos resistores, osea desde la perspectiva de la señal AC **estan en paralelo**

**Entonces:** 
$$\boxed{Z_{in}= R1 || R2 || \beta R_e}$$

Vemos que $R_e$ va a ser mucho mas grande que las demas resistencias, si ese es el caso podemos ignorarlo, y nos queda

$$\boxed{Z_{in} \approx  R1 || R2 }$$

#### Impedancia de SALIDA$Z_{out}$ - WIP
...
![](https://i.imgur.com/GXaADSC.png)Entonces la resistencia que ve la carga hacia atras es
...
* Lo que se ve detras del emitter follower es una impedancia achicada $\beta$ veces, por que eso es lo que hace el emitter follower! detras del emitter follower esta
	...
	* $Z_{in}$ de la señal de entrada
* Desde la perspectiva de la base, viendo hacia atras, $R_1||R_2||Z_{in}$, desade la perspectiva del emisor, estos son $\beta$ veces mas chicos, osea $\frac{R_1||R_2||Z_{in}}{\beta+1}$ 
 
 _Nota:es $\beta +1$ por que estrictamente, $I_e=I_c + I_b= I_b*\beta + I_b$_

$$\boxed{\huge{Z_{out}=\frac{R_1||R_2||Z_{in}}{\beta+1}||Re}}$$
...
[fuente, ver!](https://www.youtube.com/watch?v=n_bBF67mqk8&t=22s)
**nota iprtate**  e voltae ivd es un impt ipednc del ommn emitter po ue liia la maoria de la corriente que puede entrar po a base a orrinte  pede suinitrar e amplicorcon pacia de tension esta determinadaporsu impedancia de salida ieent rent e la ce para no quemar el transistor![fuente, ver!](https://www.youtube.com/watch?v=n_bBF67mqk8&t=22s)
**nota importante:** 
* el voltage divider es un imput impedance del common emitter, por que limita la mayoria de la corriente que puede entrar por la base!
...
* La corriente que puede suministrar este ammplificador con pca caida de tension esta determinada por su impedancia de salida!
* Quiesent current es la suficiente para no quemar el transistor!**noa** ete sistema usa el mismo fundamento que el amplificador de tension_


UPodemos usar nuestro emitter-follower para **reducir la impedancia de una señal AC**

**ATENCION:**
:warning:Un transistor **no puede amplificar  la parte negativa de la onda** (llega a cutoff, pero no puede proveer tension negativa!)

Eso implica que tenemos que hacer que nuestra señal 
1. sea positiva cuando llegue al trasitor ste ecia d los $vara e.  creamos una tension de referencia en la base emitter-follower con un valor de $\frac{_{}}{}$  aa peritamn e a oda_simpeente conn **voltage divider uando  y 4. **Acoplamos** nuestra señal AC a esa tension de referencia con un **coupling capacitor**. 



## The current source - WIP

[fuente 2](https://tinyurl.com/y7c6umte)

## The common-emitter amplifier - WIP
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE5MzMzNjE2NDYsMjk5Mzc2OTY3LDEzOT
UyOTQxNDIsNzQ5MTkyNjA2LDUyMTY2NTY1LC0yMDI2MDAxNDQx
LDc3OTA1MzQ4OCwxNDk5MTg4NjkyLDE0NTI2MTcwMTQsLTE2ND
cxNTA5NDMsLTY5ODk4MTY4NSwxMDg4NTk3MTY3LC0xNzcwODYz
NzI3LDE3ODc3ODQ3NDMsMTI5Mjc5MjM1OSwyMDgyNDQ4ODYzLD
kxNTY4ODU5LC0xMTAyMjExNzAzLC00MTIwNjcxMDEsLTEzOTI0
MDU4MjVdfQ==
-->