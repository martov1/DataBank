...
Despejamos $L$
$$L=\frac{V_{in}-V_{out}}{i_{max}}*T*D$$

**Notamos:**
Es trivial ver que el ripple no esta montado a ninguna otra corriente (por que$i_{min}=0$) por lo que
$$i_{outavg}=\frac{\Delta i$$
Que  ya que como le estoy dando besitos al $0$, no hay una corriente de base sobre la que el riplple va montado, sino que el ripple es toda la corriente que hay. entonces como el ripple es simetrico
=\frac{i_{max}}{2}
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwNzEzNTA3NTksLTI4MTAxNzUxMCwtMT
