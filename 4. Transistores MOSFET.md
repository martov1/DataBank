...
que por ley de ohm **(trivial)** es lo mismo que

$$(2) \quad V_{fuente}=I_d*R+V_{ds}$$

**Despejamos** $I_d$

$$(3) \quad I_d= \frac{-1}{R}V_{ds}+\frac{V_{fuente}}{R}$$

**Entonces:**
Esto implica que  si $V_{fuente}$ y $R$ fueran **consantes** (que es lo normal) entonces  $I_d$ depende unicamente de $V_{ds}$

Podemos entonces graficar todos los posibles valores de corriente $I_d$ en base a la caida de tension que pudiera tener el transistor!

esta va desde la corriente maxima (cuando $V_{ds}=0)$
$$(3) \quad I_{dmax}= 0+\frac{V_{fuente}}{R}$$
![](https://i.imgur.com/0zJZCR0.png =400x)

Hasta corriente $I_d=0$ que es cuando $V_{ds}=V_{fuente}$





## Ejemplos en circuitos  - WIP
...
* La **corriente maxima** que puede **proveer el circuito** $I_d$
* La tension $V_{gs}$

### Circuito siempre en zona de saturacion

![](https://i.imgur.com/TBibpXp.png)

 [Simulacion](https://tinyurl.com/ycbbm9gl)

 **este circuito puede proveer corriente infinita**
 Vemos que el grafico del transistor nunca podria cumplir con $I_d\to \infty$ como tendria que hacer en la zona ohmica con un $R_{dsOn}$ pequeño, entonces si o si pasa a saturacion

![](https://i.imgur.com/ICl2YFx.png)
...
![](https://i.imgur.com/eO4WkV3.png)[Simulacion](https://tinyurl.com/y843wu3m)

En este circuito la corriente $I_{dmax}$ esta limitada a un cierto valor por un resistor.
En nuestro grafico podemos elegir un $V_{gs}$, si elegimos un $V_{gs}$ chiquito entonces nuestro $I_d<I_{dmax}$, es decir que el transistor estara en **saturacion** por que **puede proveer menos corriente que la maxima**
![](https://i.imgur.com/hzA8VfJ.png)
A medida que aumentamos $V_{gs}$
...
## Resistencia variable y load line
...
**RECORDAMOS:** para cada $V_{gs}$ **fijo** se respeta que  la relacion entre $V_{ds}$ y $I_d$ es una **recta** y **ese es el comporamiento de la ley de ohm**.

**Entonces:**
$$\boxed{\frac{V_{ds}}{I_d}=R_{dsOn} \quad \quad (\text{para un}  V_{gs} \text{ fijo }) }$$

Ahora bien, tengamos en cuenta **un circuito donde queremos aplicar el MOSFET como resistor variable**
![](https://i.imgur.com/3rZX0Xs.png =400x)

**1) Primero observemos:** Algunos escenarios hipoteticos de que le pasaria al circuito con valores extremos de $R_{dsOn}$
* $\color{orange}R_{dsOn}=0$, _osea el mosfet conduce totalmente_
	*  La **corriente** $I_d$ de este circuito sera
	 **Maxima** y sera limtada solo por **el circuito ALREDEDOR DEL MOSFET** (_si no hay resistencia el mosfet no incide en la corriente_)
		* Su valor sera $\frac{V_{cc}}{R_{load}}$
	* La tension $V_{ds}$ sera $0$ (_si no hay resistencia no hay caida de tension_)
* $\color{green}R_{dsOn}\approx\infty$, _osea como si el mosfet no conduce_
	*  La **corriente** $I_d=0$ 
	* La tension $V_{ds}$ sera $V_{cc}$

**2) Ademas sabemos que:**
1. Si el mosfet esta en **zona lineal** entonces **se comporta como una resistencia** con algun valor de $R_{dsOn}$
2. $R_{dsOn}$ tiene que estar entre $\color{green}\infty$ y $\color{orange}0$
3. Entre $\color{orange}0\color{black}<R_{dsOn}<\color{green}\infty$, La relacion entre $V_{ds}$ y $I_d$ tiene que ser $\color{magenta}\text{lineal}$ por que **respeta la ley de ohm**

**3) Entonces:**
podemos dibujar una linea que
* **EMPIEZA desde** la condicion cuando $\color{orange}R_{dsOn}=0$, osea cuando el mosfet conduce y la corriente solo esta **limitada por el resto del circuito alrededor del mosfet.**
* **TERMINA en** la condicion cuando $\color{green}R_{dsOn}\approx\infty$, es decir cuando $I_d=0$ y $V_{ds}=V_{cc}$, es decir cuando el mosfet no deja pasar nada de corriente.

$\color{magenta}\text{Esta linea representa todas las} \\ \text{ combinaciones \\  posibles de }I_d \text{ y } V_{ds}$ si el MOSFET **fuera un resistencia variable, para todos los valores posibles de** $R_{dsOn}$

![](https://i.imgur.com/y8bsZqx.png)
**CONCLUSION:**
Vemos entonces como para **cada circuito en particular que rodea al mosfet** puedo construir un grafico que,  estando en la **zona lineal** me indica
* **Todas las posibles combinaciones de** $I_d$ y $V_{ds}$ para este circuito, $\color{magenta}\text{que caen en la lineal violeta}$
* **Para cada valor de** $V_{gs}$ **puedo determinar que valores de**  $V{ds}$ y $I_d$ tendre viendo donde toca el grafico toca a $\color{magenta}\text{lineal violeta}$ para ese $V_{gs}$
* Puedo obtener $R_{dsOn}$ para cada $V_{gs}$
	*  despejando $\frac{V_{ds}}{R_{dsOn}}=I_d$
...
![](https://i.imgur.com/44fqb2l.png =400x)
# Amplificacion - WIP
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjM2MjQ2ODM1LDI5Njk3NzkwNSwtMzY2Mj
E2Mjg1LDExNDM4OTY2MDIsMTA5ODY2NzI4OCw2MTYyMDAwMDAs
LTE3MzQ1NTQxNDcsLTMxMzUyODk4LDc4NTc1MDUyLDQ0ODIwOT
c0MCwtMTcwNjY5NDQxNSwtMTcwNjY5NDQxNSw5MTk4MzcwNDIs
MjE0MjIwNTkzNiw1NTI0NTMyNTgsMTk0NjU3MjczOSwtMTE0MD
ExODcwNywtNTE0MDI2MjY0LC0yMjgzODgxNjgsLTEwMTI1NTc4
MjZdfQ==
-->