...
* **Desde** la condicion cuando $\color{orange}R_{dsOn}=0$, osea cuando la corriente solo esta **limitada por el resto del circuito alrededor del mosfet.**
* **Hasta** la condicion cuando $\color{green}R_{dsOn}\approx\infty$, es decir cuando $I_d=0$ y $V_{ds}=V_{cc}$, es decir cuando el mosfet no deja pasar nada de corriente.

$\color{magenta}\text{Esta linea representa todas las} \\ \text{ combinaciones \\  posibles de }I_d \text{ y } V_{ds}$ si el MOSFET **fuera un resistencia variable**
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTI5NDI4OTcxMSw1NTI0NTMyNTgsMTk0Nj
