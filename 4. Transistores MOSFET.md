...
**Entonces** en el punto $x$ donde $V_{cs}(x)=V_{g}-V_{th}$ la equacion $(3)$ se transforma en la  $(6)$ y por consiguiente desde ese punto $x$ en adelante el capacitor no tiene carga por que $Q=0$

**CRITICO:** Eso significa que desde $x$ hasta el **drain** hay un **depletion zone** sin electrones libres.  


![](https://i.imgur.com/3OZTktB.png)

Para que la corriente pueda atravesar ese **depletion zone** entre $X$ y **drain** que queda donde no hay electrones (por que $Q(x)=0$), se genera la caída de potencial entre $V_{ds}-V_{channel}$ similar a la de un diodo.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTkwMDAyODc1NywtMTQ2MTYxMTUzLDEwOD
Y4ODA0NjcsNDQwNjY1ODMyLDY0MjY5NTY3OSw3MTIzMzkxMTgs
LTExOTU3Mjg5MTgsMTQ0NTUyNTQzNSwxMDMzMTYyMDQsLTExNj
