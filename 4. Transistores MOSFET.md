...
4. Ademas si en algun momento $V_{g}-V_{th}=V_{channel}(x)$ el canal se cierra y no hay carga en esa coordenada $X$ del capacitor.
	* **Por que** $Q=C([V_{g}-V_{th}]-V_{channel}(x))=0$
5. Entonces en esa seccion del canal teoricamente **no es posible la conduccion**

**EL PINCH-OFF:**
**Busquemos el momento en el que el canal se hace finito hasta cerrarse**
Estoy buscando el lugar de $X$ donde la carga del capacitor en ese lugar es  $Q=0$
$$Q=C(V_{cap}(X)-V_{th})=0$$

Eso solo sucede cuando **la resta sea cero**, entonces
$$V_{cap}(x)=V_{th} \Rightarrow Q(x)=0$$

Reemplazando $V_{cap}$ con la equacion $(2)$
$$V_{g}-V_{channel}(x) =V_{th} \Rightarrow Q(x)=0$$

Osea
$$V_{channel}(x) =V_{g}-V_{th}  \Rightarrow Q(x)=0$$

**conclusion 1:**
Si en algun punto $X$ se da que $V_{channel}(x) =V_{g}-V_{th}$ entonces en ese punto $X$ el capacitor no tiene cargas $Q$

$$\boxed{V_{channel}(x) =V_{g}-V_{th} \Rightarrow Q(x)=0}$$
**Conclusion 2:**
A) Sabemos que: Por el punto 1,  $V_{d}<V_{channel}(x)<V_{s} \quad \forall x$ 
B) entonces si se diera que  $V_{ds}>V_{g}-V_{th}$
C) Por la  concusion 1 existe un punto $X$ en donde $Q=0$


![](https://i.imgur.com/3OZTktB.png)
Si no es posible la conducción entonces **la caída de tension no continua**. Veamos el grafico de caida de tension a lo largo de $X$
![](https://i.imgur.com/Dwkysy8.png)
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE2NzA0OTU3NDEsNzQ5NDE3Nzg1LDE1Mz
QzMDYzMDUsMjIxNDAwMzQ1LC0yMDkwNzE5NTgxLDEyMzc0NjEz
OTUsMTYwNjM3MjQzMiwtMzA4OTk3NjgxLDEwMzUwNTI3MTksMz
g3Nzg1NDIzLDc2MjM1MzI0Niw0NDIzNTczNDAsLTE5NTQzNzA4
NzksLTExNjYyOTYwNDcsLTIxMzM0MTUsMjEzMjE5ODI3NSwtNz
A3Mzk4NTYsLTMxNzgwNDY4MCw4NzYyOTk4NTMsODc1MTI4Mzk0
XX0=
-->