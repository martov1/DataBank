...
### Capacitancia $C_{gd}$ y su dependencia a $V_{ds}$ 
[fuente](https://www.youtube.com/watch?v=1XqyrAgMQVc&t=258s)
[fuente 2](https://www.vishay.com/docs/68214/turnonprocess.pdf)
...
$$Q_{dg}=C_{dg} V_{dg}$$
Pero cuando quiero prender mi mosfet, mi capacitor $C_{dg}$ pasa por dos estados
* Estado inicial: Transistor apagado 
	* $V_{dg-inicial}=600V$
* Estado final: Transistor prendido
	* $V_{dg-final}=-9V$

**Entonces primero**  tengo que descargar el capacitor, es decir removerle la carga $Q$ que actualmente tiene
$$Q_{dg}=C_{dg} V_{dg-inicial}$$


**Luego** tengo que cargarlo al reves
$$Q_{dg}=C_{dg}V_{dg-final}$$

**En total** tengo que movilizar la suma de las dos cargas
$$Q_{dg}=C_{dg} V_{dg-inicial}+C_{dg}V_{dg-final}$$

$$\boxed{Q_{dg}=C_{dg} (V_{dg-inicial}+V_{dg-final})}$$

**CONCLUSION:**
La cantidad de carga $Q_{dg}$ que necesitamos proveer **varia fuertemente con la tension** $V_{d}$ entonces **la capacidad percibida depende de la tension**
...
Pero aun nos falta calcular el otro capacitor $C_{gs}$, afortunadamente esta capacitancia tiene en sus extremos $V_{g}$ y $V_{s}$, que tienen estados muchos mas sencillos
...
$Q_{sg}=C_{sg} (V_{s}-V_{g})$
$Q_{sg}=C_{sg} (0-V_{g})$

$$\boxed{Q_{sg}=C_{sg} V_{g}}$$


Entonces, sumando $Q_{sg}$ que calculamos recien y $Q_{gd}$ podemos obtener la **carga total** $C_{t}$ necesaria para switchear el transistor totalmente.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTYxNDE3Mzc1MSwyNTIwODU4MzcsLTg5NT
g2MjEzMiwtODI5MTE5NTQ3LDYxMzk0NDI4NiwtMTMwNzI3NTYs
LTE1NTM5MDMwNTksLTE2NjYxMTEwNTEsMjIwNTE2NjI5LC0yMT
M3MTQ3NDk5LDg2NjMwMDUwNiwtMTg1OTA1Njk1MiwtMTE5Nzk5
NjUxNSwtNDMyNTkyNTcsMjAzNzA3Mjg5OF19
-->