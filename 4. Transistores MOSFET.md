...
*
5. **Ademas** si en algun momento $V_{g}-V_{th}=V_{channel}(x)$ el canal se cierra y no hay carga en esa coordenada $X$ del capacitor.
**Por que** $Q=C([V_{g}-V_{th}]-[V_{g}-V_{th}])=0$


**ENTONCES:**
6.  Imaginemos que se da que  $V_{ds}>V_{g}-V_{th}$
7. Por la equacion $(4)$ existe un punto $X$ en donde $V_{channel}=V_{g}-V_{th}$, llamamos a ese valor $V_{dsSat}$
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTYyNTAxOTc4Myw2NDI2OTU2NzksNzEyMz
