...
**EL PINCH-OFF:**
Busquemos el momento en el que el canal se hace finito hasta cerrarse, es decir el lugar de $X$ donde $Q=0$
$Q=C(V_{cap}(X)-V_{th})=0$
Eso implica
$V_{cap}=V_{th}$
Reemplazando con la$(2)$
$V_{g}-V_{channel}(x) =V_{th}$
Osea
$V_{channel}(x) =V_{g}-V_{th}$


 $V_{ds}>V_{gs}-V_{th}$, por el punto 1,  en algun punto $X$ se dara que $V_{ds}=V_{gs}-V_{th}$, eso implica que 


 por el punto 1. que $V_{cap}<V_{th}$ en algun punto $X$, y en ese punto entonces por la equacion $(1)$ la carga sera cero $Q=0$
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE2NTg2NTM5MTMsMjIxNDAwMzQ1LC0yMD
