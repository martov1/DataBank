...
**Notamos entonces que** si en algun momento $V_{g}-V_{th}=V_{cs}(x)$ el canal se cierra y no hay carga en esa coordenada $X$ del capacitor.
 $$\boxed{(5) \quad Q=C([V_{g}-V_{th}]-[V_{g}-V_{th}])=0}$$
...
4. **Entonces** por la equacion $(3)$ en ese  punto $X$ tendremos un depletion region donde  $Q=0$ 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTMyODg1MDAwNSwtMTQ2MTYxMTUzLDEwOD
