...
>$$ \quad V_{channel}=\frac{V_s+V_d}{2}$$
> Entonces
>$$(8) \quad V_{cap}=V_g-\frac{V_s+V_d}{2}$$
...
$$(2) \quad V_{cap}=V_g-\frac{V_s+V_d}{2}$$

**Notamos que:**
Esas equaciones _suponen_ que el capacitor es **homogeneo y simetrico**, pero notamos que ese **no es el caso.**, entonce ya no podemos considerar a $V_{cap}$ como la tension en una placa $V_g$ menos el promedio de las tensiones en la otra placa.

**Notamos que:** 
1. **Trivial:** La tension $V_{ds}$ en el  canal cae desde $V_{d}$ hasta $V_{s}$ a lo largo de la distancia $X$ del canal
2. Entonces  $V_{cap}$ **baja a medida que avanzamos en** $X$
3. Entonces la cantidad de carga $Q$ va bajando a medida que avanzamos en $X$ y cae $V_{cap}$
4. Ademas si en algun momento $V_{cap}<V_{th}$ el canal se cierra.
	* Recordamos $Q=C(V_{cap}-V_{th})$
	* **Entonces** $Q=C(V_{th}-V_{th})=0$

**Como consecuencia del punto 4 y 1:**
1. Cuando  $V_{ds}=V_{gs}-V_{th}$
2. $V_{cap}$ debera atravesar $V_{th}$ en algun punto $X$
3. En ese punto el channel se queda con carga neutra (el P-type se queda con sus agujeros llenos pero sin carga adicional para mover corriente)
4. Nos quedamos sin carga $Q$ en el capacitor en cierta posicion $X$.
...
---
Grafiquemos ahra la cantidad de carga $Q$ segun la posicion $X$ en las placas  del capacitor. 
$$Q_{libres}=C(V_{cap}-V_{th})$$

Vemos que tiene sentido que cuando  $V_{cap}=V_{th}$ se de que la carga $Q=0$.
$$Q_{libres}=C(V_{th}-V_{th})=0$$
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIxMzM0MTUsMjEzMjE5ODI3NSwtNzA3Mz
k4NTYsLTMxNzgwNDY4MCw4NzYyOTk4NTMsODc1MTI4Mzk0LC0x
NDcxNjI5ODMzLDUyMTkzNjc1NCw0NTU3MDAzOTksLTEwMjUzMT
A0MSwtMTAwMTUwNTYzMiwxOTgxNjA2MDUxLC0yMTAyOTYyMTU5
LC0xMjQ0NDQ0NjIwLDg3NjM5MDc3Niw4NzY0NTM0NDYsLTgzMz
A5MjE3NCwxNjc1ODAyODgwLC0xNDQxMTA4NjkyLC0xOTQ4MjUx
ODc2XX0=
-->