...
**Notamos entonces que** si en algun momento $V_{g}-V_{th}=V_{cs}(x)$ el canal se cierra y no hay carga en esa coordenada $X$ del capacitor.
 $$\boxed{Q=C([V_{g}-V_{th}]-[V_{g}-V_{th}])=0}$$

### Region de pinch-off

Imaginemos que se da que  $V_{ds}>V_{g}-V_{th}$, esto desencadenara la region de **pinch-off** por las cuestiones que veremos a continuacion.

1. Por la equacion $(4)$ existe un punto $X$ en donde $V_{channel}=V_{g}-V_{th}$, llamamos a ese valor $V_{dsSat}$
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIxMDA3Njk4NjQsNDQwNjY1ODMyLDY0Mj
Y5NTY3OSw3MTIzMzkxMTgsLTExOTU3Mjg5MTgsMTQ0NTUyNTQz
NSwxMDMzMTYyMDQsLTExNjM4NDQ4MjUsNzQ5NDE3Nzg1LDE1Mz
