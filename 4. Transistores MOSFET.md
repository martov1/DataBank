...
$$V_{cs}(x)=V_{dsSat}=V_{g}-V_{th}$$
 La equacion $(3)$ se transforma en la  $(6)$ y por consiguiente **desde ese punto** $x$ **en adelante el capacitor no tiene carga** por que segun la equacion $(6)$ tenemos $Q=0$

>**CRITICO:** Eso significa que en el canal, desde $x$ hasta el **drain** hay un **depletion zone** sin electrones libres.  
...
**ENTONCES:**
La corriente $I_d$ que atraviesa el mosfet es la misma que cuando $V_{ds}=V_{g}-V_{th}$, la corriente $V_{ds}$ adicional cae en la depletion zone, que se hace mas grande a medida que $V_{ds}>V_{g}-V_{th}$, disipandose en forma de calor
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE2Njc1MTAyMjksLTE0NjE2MTE1MywxMD
