...
* $R_{dsOn}=0$, _osea el mosfet conduce totalmente_
...
* $R_{dsOn}\approx\infty$, _osea como si el mosfet no conduce_
	*  La **corriente** $I_d=0$ 
...
* **Desde** la condicion cuando $R_{dsOn}=0$, osea cuando la corriente solo esta limitada por el resto del circuito alrededor del mosfet.
* **Hasta** la condicion cuando $R_{dsOn}\approx\infty$, es decir cuando $I_d=0$ y $V_{ds}=V_{cc}$, es decir cuando el mosfet no deja pasar nada de corriente.

$\color{magenta}\text{Esta linea representa todas las combinaciones posibles de}$ **representa todas las combinaciones posibles de**  que puede haber si el MOSFET **fuera un resistencia variable**
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTMxODQ5NDAxMiwxOTQ2NTcyNzM5LC0xMT
