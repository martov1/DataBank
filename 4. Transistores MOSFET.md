...
Hago disributiva y reordeno **quedando** entonces la equacion que relaciona $I_d$, $V_{ds}$ y $V_{gs}$
$$\boxed{I_d\approx k((V_{gs}-V_t)V_{ds}-\frac{1}{2}V_{ds}^2)}$$

## Equacion y la zona lineal /ohmica- WIP
...
## Equacion y la zona de saturacion - WIP
...
Para comprender mas en detalle este fenomeno, **recordamos** la ecuacion de carga del capacitor MOS y tambien su tension. las mismas fueron derivadas en la seccion de "equacion del mosfet".
...
Como la tension no sera homogenea no podemos usar el promedio de la tension $\frac{V_s+V_d}{2}$, sino que tenemos que hablar una funcion  $V_{channel}$ que varia con la posicion $X$ a lo largo del canal.
La equacion quedaria asi. 
$$(2) \quad V_{cap} = V_{g}-V_{channel}(x)$$
Esto implica que la carga del capacitor varia segun la posicion en $X$
$$(1) \quad Q_{libres}(x)=C(V_{cap}(x)-V_{th})$$

**Reemplazando** nos queda

$$ Q_{libres}(x)=C(V_{g}-V_{channel}(x)-V_{th})$$

$$\boxed{(3) \quad Q_{libres}(x)=C([V_{g}-V_{th}]-V_{channel}(x))}$$

**Notamos que:** 
1. **Trivial:** La tension $V_{ds}$ en el  canal cae desde $V_{d}$ hasta $V_{s}$ a lo largo de la distancia $X$ del canal
2. Entonces  $V_{channel}$ **baja a medida que avanzamos en** $X$

 $$\boxed{(4) \quad V_{d}<V_{channel}(x)<V_{s} \quad \forall x}$$
**Entonces:**
3. La cantidad de carga $Q$ va **subiendo** a medida que avanzamos en $X$ y $V_{channel}$ se hace mas chiquito, por que vemos que $V_{channel}$ **esta restando** en $(3)$
$$Q_{libres}(x)=C([V_{g}-V_{th}]-V_{channel}(x))$$
**Ademas:**
4. Ademas si en algun momento $V_{g}-V_{th}=V_{channel}(x)$ el canal se cierra y no hay carga en esa coordenada $X$ del capacitor.
**Por que** $Q=C([V_{g}-V_{th}]-[V_{g}-V_{th}])=0$


**ENTONCES:**
1.  Imaginemos que se da que  $V_{ds}>V_{g}-V_{th}$
2. Por la equacion $(4)$ existe un punto $X$ en donde $V_{channel}=V_{g}-V_{th}$, llamamos a ese valor $V_{dsSat}$
3 Por la equacion $(3)$ en ese  punto $X$ tendremos un depletion region donde  $Q=0$ 


![](https://i.imgur.com/3OZTktB.png)
Para que la corriente pueda atravesar el espacio $X$ que queda donde no hay electrones (por que $Q(x)=0$), se genera la caída de potencial entre $V_{ds}-V_{channel}$ similar a la de un diodo.

Esa caida **no impulsa corriente adicional** de la misma manera que la ciada de un diodo no lo hace.

**Entonces**  la corriente $I_d$ que atraviesa el mosfet es la misma que cuando $V_{ds}=V_{g}-V_{th}$, la corriente $V_{ds}$ adicional cae en la depletion zone, que se hace mas grande a medida que $V_{ds}>V_{g}-V_{th}$

![](https://i.imgur.com/1AJ0mtq.png)
###  Conclusion y equacion
A partir de  $V_{ds}=V_{g}-V_{th}$ la corriente queda trabada en su ultimo valor.


Aumentar $V_{ds}$ solo 
* Aumenta una **depletion zone** 
* Esta se va comiendo la diferencia entre $V_{g}-V_{th}$ y $V_{ds}$, y esa caida no aporta al movimiento de la corriente (similar a como la caida de $0.7V$ de un diodo no respeta la ley de ohm y no hace circular una corriente )

Recordando la formula para $I_{ds}$ y evaluandola cuando $V_{ds}=V_{g}-V_{th}$ obtenemos el valor al que queda clavada la corriente, aun cuando $V_{ds}$ supere ese valor.
...
$$I_d (V_{gs}) \approx k((V_{gs}-V_t)(V_{g}-V_{th})-\frac{1}{2}(V_{g}-V_{th})^2)$$


## Curvas de MOSFET - WIP
[fuente 1](https://www.youtube.com/watch?v=H7Gdz4QTvUU&t=266s)
[fuente 2](https://www.youtube.com/watch?v=pP7WWM63Upc)
...
## Amplificador (zona de saturacion)- WIP
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTMzMjk2NDgzNSwtMTE5NTcyODkxOCwxND
Q1NTI1NDM1LDEwMzMxNjIwNCwtMTE2Mzg0NDgyNSw3NDk0MTc3
ODUsMTUzNDMwNjMwNSwyMjE0MDAzNDUsLTIwOTA3MTk1ODEsMT
IzNzQ2MTM5NSwxNjA2MzcyNDMyLC0zMDg5OTc2ODEsMTAzNTA1
MjcxOSwzODc3ODU0MjMsNzYyMzUzMjQ2LDQ0MjM1NzM0MCwtMT
k1NDM3MDg3OSwtMTE2NjI5NjA0NywtMjEzMzQxNSwyMTMyMTk4
Mjc1XX0=
-->