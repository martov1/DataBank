...
$\color{magenta}\text{Esta linea representa todas las} \\ \text{ combinaciones \\  posibles de }I_d \text{ y } V_{ds}$ si el MOSFET **fuera un resistencia variable**, para todos los valores posibles de $R_{}$

![](https://i.imgur.com/y8bsZqx.png)
**CONCLUSION:**
Vemos entonces como para **cada circuito en particular que rodea al mosfet** puedo construir un grafico que,  estando en la **zona lineal** me indique 
* Todas las posibles combinaciones de $I_d$ y $V_{ds}$ para este circuito, $\color{magenta}\text{que caen en la lineal violeta}$
* **Para cada valor de** $V_{gs}$ **determinar que valores de**  $V{ds}$ y $I_d$ tendre
* Obtener $R_{dsOn}$ para cada $V_{gs}$
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwNTgyNDg3NTUsNTUyNDUzMjU4LDE5ND
