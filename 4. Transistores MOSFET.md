...
**Recordamos:**
A medida que $V_{ds}$ aumenta, se forma un depletion region en el drain y el canal se hace triangular.
![](https://i.imgur.com/rJRHIcr.png =300x)
Anteriormente derivamos una equacion que nos indica la cantidad de electrones libes en cada coordenada $x$ del capacitor, donde $V_{cs}(x)$ indica la tension entre ese punt $x$ y el source.
$$\boxed{(4) \quad Q_{libres}(x)=C([V_{g}-V_{th}]-V_{cs}(x))}$$
Si en algun momento  $V_{cs}=[V_{g}-V_{th}]$ entonces la equacion se anula y no hay carga en el capacitor, esto es PINCHOFF

$$ \quad Q_{libres}(x)=C([V_{g}-V_{th}]-[V_{g}-V_{th}])=0$$
# Curvas del MOSFET
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEwOTY2Njc4ODcsLTM1NjkzMTQ0MywxOD
QwNzk0MTQ3LDc1MzM2NDc3NiwtMTI3NDY3NzIzMywxMjEwNjI1
MzM1LC0xODcwNDU1ODExLC05MTQyNzE5NzksLTE4NTc5NDY1OT
AsLTEzMTQwNDU5ODIsMTAwNjAwMzU5OCw4MDk2MTc4MTksLTIw
MjcxNjY4MDAsLTEyODMwNzY0NSwxODUzNjY5MjA4LDYxNzgyNz
AwOCw2NTY1OTkzMTksLTY0NjI3NzcsMjk2OTc3OTA1LC0zNjYy
MTYyODVdfQ==
-->